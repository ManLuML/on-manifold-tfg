...
Supported Models:
    - JiT: jit-b-16, jit-l-16, jit-h-16
    - DiT: dit (facebook/DiT-XL-2-256)
    - SiT: sit (SiT-XL/2)
    - PixelFlow: pixelflow (ShoufaChen/PixelFlow-Class2Image)
...
Usage:
    # Evaluate JiT-H/16 (uses default sampling_method from config)
    uv run python experiments/imagenet_no_guidance.py --model jit-h-16

    # Evaluate JiT-H/16 with specific sampling method
    uv run python experiments/imagenet_no_guidance.py --model jit-h-16 --sampling_method euler

    # Evaluate DiT (defaults to ddpm; pass --sampling_method ddim for DDIM)
