...
def load_sit_denoiser(
    ...
    Args:
        ...
        from_pretrained: HuggingFace Hub model ID (e.g., "mit-han-lab/SiT-XL-2-256").
        device: Target device.
    ...
    Example:
        ...
        >>> denoiser = load_sit_denoiser(checkpoint_path="checkpoints/SiT-XL-2-256.pt")

        >>> # All channels mode for TFG research
        >>> denoiser = load_sit_denoiser(
        ...     checkpoint_path="checkpoints/SiT-XL-2-256.pt",
    ...
    if checkpoint_path:
        ...
        # Note: SiT checkpoints are hosted differently than DiT
        model_info = {
            "mit-han-lab/SiT-XL-2-256": {
                "url": "https://huggingface.co/mit-han-lab/SiT-XL-2-256/resolve/main/SiT-XL-2-256.pt",
                "filename": "SiT-XL-2-256.pt",
                # SHA256 should be verified when official checkpoints are available
                "sha256": None,  # Will skip verification if None
            },
            # Fallback to original SiT release (official willisma/SiT repo)
        ...
        cache_dir = os.path.join(os.path.expanduser("~"), ".cache", "jit-tfg", "sit")
        ...
        # Note: weights_only=False is required for SiT checkpoints which use
        # a custom format. Safety is ensured by SHA256 verification above.
