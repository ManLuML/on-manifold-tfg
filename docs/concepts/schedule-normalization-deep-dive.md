...
### 4.1 Flow Matching Implementation (`UnifiedSampler._get_schedule_value` in unified_sampler.py)

```python
def _get_schedule_value(self, base_value, schedule, t, normalize=False):
...
| Recurrence | `_predict_xt()` (DDPM posterior) | re-sample z_t at same t with fresh noise | ⚠️ Different formula |
...
**Our Implementation (`UnifiedSampler._euler_step` / `_heun_step`, x-space guidance branch in `unified_sampler.py`)**:
```python
...
**Note**: This transformation is **heuristic**. The scaling factors are not
derived from first principles for flow matching — they are an empirical mapping
of TFG's DDPM coefficients onto the flow-matching time convention (see
Appendices C and D for a comparison against the theoretically derived
`(1-t)/t` scaling of Flow Guidance).

### B.4 Recurrence Mechanism

**Original TFG (edm/methods/base.py)**:
```python
...
**Our Implementation (`UnifiedSampler._compute_flow_guidance_deltas`, recurrence branch in `unified_sampler.py`)**:

Between recurrence iterations, the flow-matching path performs a **complete
re-sample at the same timestep t** with a fresh noise realization: it estimates
the clean sample from a fresh velocity prediction, then re-noises it back to
time t along the flow-matching interpolation.
...
# (optional clipping of x_est via config.clip_x0)

# Complete re-sample with new noise at the same t
...
| Item | DDPM | Flow Matching |
|------|------|---------------|
| Mechanism | Re-noise x_{t-1} → x_t (DDPM posterior reversal, one step back) | Re-sample z_t at the **same** t from the x estimate with fresh noise |
| Noise scale | `√(1 - α_t/α_{t-1})` | `(1-t)` (interpolation noise coefficient) |
| Meaning | Posterior variance | Fresh draw from the interpolation path q(z_t \| x̂) |
...
```python
# UnifiedSampler._dit_tfg_step / _dit_tfg_ddpm_step (guidance application)
x_prev = x_prev + delta_t / alpha_t**0.5 + delta_0 * alpha_prod_t_prev**0.5

# UnifiedSampler._get_dit_sigma
scheduler = (1 - alpha_prod_ts) ** 0.5  # Same as original

# UnifiedSampler._get_dit_rho / _get_dit_mu
normalizer = len(scheduler) / scheduler.sum().clamp_min(1e-8)  # Same as original
```
...
| Recurrence | ✓ | ⚠️ Re-sample at same t (different formula) | Medium |
...
4. **Recurrence**: ⚠️ **Uses different formula**
   - DDPM posterior reversal vs complete re-sample at the same t with fresh noise
   - Functionally same purpose (repeat the step from a refreshed noisy state)
