...
2. **Does the hierarchy hold across guidance tasks?** The paper shows the same
   ordering extends to style transfer, but only class guidance and the inverse
   problems (deblur / super-resolution) are reproducible with the released
   scripts — style-transfer code is not part of this release.
