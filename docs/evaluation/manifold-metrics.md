...
**Hypothetical predictions** (experiments pending --- actual results may differ):

| Model | Pred | Validity \\(\uparrow\\) | **Precision** \\(\uparrow\\) | **Density** \\(\uparrow\\) | **Coverage** \\(\uparrow\\) | Src FID \\(\downarrow\\) | Tgt FID \\(\downarrow\\) |
|-------|------|-----------|---------------|-------------|-------------|----------|----------|
| JiT-H | x | ~72% | **~0.90** | **~1.8** | **~0.85** | **~45** | ~62 |
| DiT-XL | \\(\epsilon\\) | ~71% | ~0.68 | ~0.9 | ~0.72 | ~78 | ~65 |
| SiT-XL | v | ~70% | ~0.74 | ~1.1 | ~0.78 | ~72 | ~64 |

If confirmed, Validity would be comparable across prediction targets while **Precision reveals the manifold fidelity gap**.

### Testable Hypotheses

1. **H1: "x-prediction achieves highest manifold Precision under TFG"** --- Would provide direct evidence that x-pred guided samples stay on the real data manifold
2. **H2: "High validity with low Precision indicates adversarial-like artifacts"** --- Would suggest e/v-pred's validity is inflated by off-manifold samples (though mode collapse is an alternative explanation)
3. **H3: "x-prediction degrades gracefully"** --- When guidance fails, x-pred maintains high Precision (realistic images of wrong class) while e-pred collapses
4. **H4: "Manifold fidelity scales with dimension"** --- Toy experiment Precision curves may show x-pred stability vs. e/v-pred degradation

### Priority Summary

| Priority | Metric | Role | Compute Cost |
|----------|--------|------|-------------|
| 1 | **Precision** (DINOv2) | Core: manifold fidelity | Medium |
| 2 | **Density** (DINOv2) | Core: manifold depth | Free (with Precision) |
| 3 | **Coverage** (DINOv2) | Core: diversity / mode collapse detection | Free (with Precision) |
| 4 | **Realism Score** | Analysis: per-sample visualization | Medium |
| 5 | FD-DINOv2 | Complement: FID cross-validation | Low |
| 6 | Multi-Classifier Agreement | Complement: adversarial detection | Low |

**Minimum recommendation**: Precision + Density + Coverage (computed together via `prdc`, strongest impact on paper narrative). Recall is also computed by `prdc` at zero cost and should be reported for completeness.
