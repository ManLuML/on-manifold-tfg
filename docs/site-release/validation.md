...
## 2026-09-02 outcome-header normalization

Pull request [#8](https://github.com/ManLuML/on-manifold-tfg/pull/8) aligned slide 12/18 with the standard body-slide template: cyan uppercase `Guidance outcomes` kicker, `max-w-6xl` header alignment, and the same 48px one-line title treatment used elsewhere. The Google Slides frame and caption region was reduced from 1320px to 1240px to make room without shrinking the 22px cumulative captions. Pages [run 33528193941](https://github.com/ManLuML/on-manifold-tfg/actions/runs/33528193941) deployed squash commit `7053576e5a6ea6c59ea55659d849b5b5f33f7d98` and passed the production verifier.

The live slide index SHA-256 is `33e6dfaadb3ea094d101d7adbdba63aa8343b4dd59a08306bb00c43836412659`. Cache-cold public-origin Chromium confirmed the standard kicker, one-line 48px title, 826.67px figure width, four non-overflowing captions, and 15.87px page-number clearance. Full regression remains 18 slides and 40 unique states with no clipping, overflow, collisions, broken images, notes, nesting, or console errors. The editable `gh-pages` source branch was fast-forwarded to `d9c1941`.
